...
async def query_nvd_api_by_date_range(session, start_date, end_date, api_key):
    ...
    # Log the URL being queried for debugging
...
def main():
    ...
    parser.add_argument('-a','--api_key', type=str, help='NVD API key for querying CVEs')
    parser.add_argument('-s','--start_date', type=str, help='Start date for CVE data pull (format: YYYY-MM-DD)')
    parser.add_argument('-e','--end_date', type=str, help='End date for CVE data pull (format: YYYY-MM-DD)')
    args = parser.parse_args()

    api_key = "8baea3b9-170e-4c2b-a4ac-39a54a18f9a8"
    ...
    try:
        ...
        print(f"An error occurred during the CVE pull process: {e}")
        logging.error(f"An error occurred during the CVE pull process: {e}")
