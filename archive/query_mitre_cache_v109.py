...
async def query_cache(conn, cve_number):
    async with conn.execute("SELECT * FROM cve_data WHERE cve_id = ?", (cve_number,)) as cursor:
        ...
        if row:
            print(f"Using cached data for {cve_number}")  # Show cache results
...
async def search_cve(session, cve_number, api_key):
    ...
    try:
        async with session.get(url, headers=headers, timeout=timeout) as response:
            ...
            print(f"Fetched data from NVD for {cve_number}")  # Show NVD query result
...
def save_to_excel(data):
    ...
    filename = f"output/cve_data_{timestamp}.xlsx"
    
    df = pd.DataFrame(data)
    df.to_excel(filename, index=False)
    print(f"Data successfully saved to {filename}")  # Show Excel save result
...
async def query_cves(cve_numbers, api_key, conn):
    ...
    async with aiohttp.ClientSession() as session:
        for index, cve_number in enumerate(cve_numbers, start=1):
            print(f"Checking cache for {cve_number}")  # Show cache check result
            ...
            print(f"Querying NVD for {cve_number} ({index} out of {len(cve_numbers)})")
