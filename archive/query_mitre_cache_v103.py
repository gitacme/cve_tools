...
async def search_cve_async(cve_number, api_key, session, conn, total_cves, index):
    ...
    print(f"Checking cache for {cve_number}")
    ...
    # If not in cache, query the NVD API
...
async def query_cves_in_parallel(cve_numbers, api_key, conn):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for index, cve_number in enumerate(cve_numbers, start=1):
            ...
            # Every 22 requests (non-cache), wait for 15 seconds to avoid hitting the API rate limit
            if index % 22 == 0:
                results = await asyncio.gather(*tasks)
                tasks.clear()  # Clear tasks before next batch

                # If none of the 24 results came from cache, sleep to avoid rate limiting
                from_cache = all(result is not None for result in results)
                if not from_cache:
