# CVE Query Script - Last 90 Days CVE Pull - Version 1.0.4
# Copyright 2024 Steve Fink
# This script pulls the last 90 days of CVE data from the NVD API by default,
# adds it to the local caching database, and outputs results to Excel.
# The user can specify a time period using --start-date= and --end-date=
...
# Function to query NVD API by date range with pagination handling
async def query_nvd_api_by_date_range(session, start_date, end_date, api_key):
    url_template = f"{BASE_URL}?pubStartDate={start_date}T00:00:00.000Z&pubEndDate={end_date}T23:59:59.999Z&resultsPerPage=2000&startIndex={{}}"
    ...
    while True:
        ...
        logging.debug(f"Querying NVD API with URL: {url}")
        ...
        try:
            async with session.get(url, headers=headers, timeout=timeout) as response:
                ...
                if total_results is None:
                    ...
                    logging.info(f"Total CVEs available: {total_results}")
                ...
                logging.info(f"Retrieved {len(vulnerabilities)} CVEs, total retrieved so far: {len(all_vulnerabilities)}")

                # If we have retrieved all results, stop
...
async def main_async(api_key, start_date=None, end_date=None):
    ...
    try:
        ...
        logging.error(f"Invalid date format or range provided: {e}")
        raise
